...
const DEFAULT_STEPS: ContactStep[] = [
  {
...
const DEFAULT_QUOTE = "What truly sets them apart is transparency. After past experiences with agencies where SEO felt like a 'black box,' it's refreshing to work with a team that provides real, clear data and explains what's working and why.";
const DEFAULT_SOURCE = "Pure Plumbing & Air — Google Review";

const ServiceContactForm = forwardRef<HTMLElement, ServiceContactFormProps>(({
  ...
  exclusivityLine = "One plumbing or HVAC company per market. Your competitors don't get in.",
  steps = DEFAULT_STEPS,
  formType,
  testimonialQuote = DEFAULT_QUOTE,
  testimonialSource = DEFAULT_SOURCE,
}, ref) => {
  return (
    <section id="contact" ref={ref} className="relative overflow-hidden">
      {/* Off-white background stops before card bleeds into footer */}
                      ...
                      >
                        <AccordionTrigger className="text-lg font-medium text-white hover:no-underline py-4">
                          <div className="flex items-center gap-4">
                            <span className="w-8 h-8 rounded-full bg-accent-blue/20 text-accent-blue text-sm font-bold flex items-center justify-center shrink-0">
                              {index + 1}
                            </span>
