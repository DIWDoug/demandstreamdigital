...
const VALID_ROUTES = new Set([  "/",  "/404",  "/about",  "/case-studies",  "/contact",  "/our-blog",  "/white-label-content-marketing",  "/white-label-gbp-seo",  "/white-label-inbound-marketing-services",  "/white-label-link-building",  "/white-label-local-seo",  "/white-label-paid-media",  "/white-label-review-management",  "/white-label-schema-markup",]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    const mapped = REDIRECTS[pathname];
    if (mapped) {
      const dest = new URL(url.toString());
      dest.hostname = canonicalHost;
      dest.pathname = normalizePath(mapped);
